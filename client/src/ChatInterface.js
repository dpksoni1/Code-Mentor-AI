...
const ChatInterface = ({ role }) => {
    ...
    const [voiceReady, setVoiceReady] = useState(false); // Flag to ensure voices are loaded
    const messageHistoryLength = 7; // Number of recent user messages to store
    const [userMessageHistory, setUserMessageHistory] = useState([]); // Store the last N user messages
   ...
   const updateUserMessageHistory = (newMessage) => {
        setUserMessageHistory(prevHistory => {
            ...
            return newHistory.slice(-messageHistoryLength); // Keep only the last N messages
        });
    };
    ...
    return (
        ...
        >
            ...
            <div
                className=" d-flex align-items-center h-auto mx-auto mb-3"
